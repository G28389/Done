...
while date<3:
    ...
    while j<1:#一天内各时刻
        ...
        for a in dc_re:
            if a[4]<ave_sdc:
                bsum_bad=bsum_bad+dict_mdc[a[0]]
                bnum_bad=bnum_bad+1
            else:
                bsum_well=bsum_well+dict_mdc[a[0]]
                bnum_well=bnum_well+1
        print(bsum_bad/bnum_bad)
        print(bsum_well/bnum_well)
        for a in dc_re:
            a[2]=(a[4]-min_sp)/(max_sp-min_sp)#标准化并付给a[2]
            dict_mdc[a[0]]=float(dict_mdc[a[0]])*0.4+float(a[2])*0.6#更新mp值
        sum_well=0
        num_well=0
        sum_bad=0
        num_bad=0
        for a in dc_re:
            if a[4]<ave_sdc:
                ...
                num_well=num_well+1
        print(sum_bad/num_bad)
        print(sum_well/num_well)
        j=j+1
    date=date+1
