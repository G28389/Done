    ...
    while j<24:#一天内各时刻
        ...
        for a in list:#取出任务的数据放进data
            if float(a[1])==date:
                if a[2].startswith(time[j]):
                    data.append(a)
        for a in data:
            dc_b[a[0]]=random.uniform(1,2)#随机生成各个dc的报价，此报价同时适用于在线和离线
        ...
        for a in offdc_sort:
            ...
            if offsumb<B:#如果没有超过预算，就可以雇佣
                ...
                offsum_buck=offsum_buck+a[1]
            else:#否则 停止招聘
                break
        print(offsum_buck/len(offdc_re))
        print(offsum_mdc/len(offdc_re))
        #随机招聘
        randomnum=1
        while randomnum<len(offdc_re):
            a=random.choice(data)
            ransum_buck=ransum_buck+a[1]
            ransum_mdc=ransum_mdc+dict_mdc[a[0]]
            randomnum=randomnum+1
        print(ransum_buck/randonnum)
        print(ransum_mdc/randonnum)
        #再进行在线招聘
        ...
        #开始招聘
            if a[1]>mbuck:#如果新来一个dc的buck值大于阈值
                ...
                if dc_b[a[0]]<mb:#如果这个人的竞价还能小于mb，就可以招聘了
                    ...
                    sum_buck=sum_buck+a[1]
                    renum=renum+1#招聘人数加一
                    bp_sum=bp_sum+a[2]

        for a in dc_re:#初始化所有sp值
            temp.append(float(a[5]))
        print(sum_buck/len(dc_re))
        print(sum_mdc/len(dc_re))
        ...
        num_low=0
        for a in dc_re:
            a[4]=math.exp(-(abs(float(a[5])-float(ctd))))#把a[4]覆盖为sp值
            if a[4]>max_sp:
                max_sp=a[4]
            if a[4]<min_sp:
                min_sp=a[4]
        for a in dc_re:
            a[1]=(a[4]-min_sp)/(max_sp-min_sp)#标准化并付给a[2]
            dict_mdc[a[0]]=float(dict_mdc[a[0]])*0.4+float(a[1])*0.6#更新mp值
        for a in dc_re:
            sum_sdc=sum_sdc+a[1]
        ave_sdc=sum_sdc/len(dc_re)#计算平均sp
        sump=0#总的实际支付
        sumbp=0
        for a in dc_re:
            a[3]=(a[1]/ave_sdc)*a[2]
        ...
        for a in dc_re:
            if a[4]<ave_sdc:
                sum_bad=sum_bad+dict_mdc[a[0]]
                num_bad=num_bad+1
            else:
                sum_well=sum_well+dict_mdc[a[0]]
                num_well=num_well+1
